...
foreach ($OU in $OUs) {
	...
	$CN          = $OU.CanonicalName
	$ParentDN    = (([adsi]"LDAP://$($OU.DistinguishedName)").Parent).Substring(7)
	$Parent      = $AllOrgUnits | Where-Object -FilterScript {$_.DistinguishedName -like $ParentDN} | Select-Object -Property CanonicalName,Name
	$ParentCN    = $Parent.CanonicalName
